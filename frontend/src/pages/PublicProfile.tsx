...
export const PublicProfile = () => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#09090b] flex items-center justify-center text-white font-mono animate-pulse">
        Carregando mochila...
      </div>
    );
  }
          ...
          <div className="flex flex-col items-center md:items-end md:flex-row gap-6 mb-12">
            ...
            <div className="flex-1 text-center md:text-left pb-2">
              ...
              <p className="text-blue-400 font-medium flex items-center justify-center md:justify-start gap-2">
                <Code2 size={16} /> @{profile.username}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-12 gap-8">
            <div className="col-span-12 lg:col-span-8 space-y-10">
              ...
              <section>
                <h2 className="text-sm font-bold uppercase tracking-widest text-slate-500 mb-6 flex items-center gap-2">
                  <Trophy size={16} className="text-amber-500" /> Projetos em Destaque
                </h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {profile.projects?.map((proj: any) => (
                    <div key={proj.id} className="bg-slate-900/40 border border-slate-800 p-6 rounded-2xl group hover:border-blue-500/50 transition-all flex flex-col justify-between">
                      <div>
                        <h3 className="font-bold text-xl mb-2 group-hover:text-blue-400 transition-colors">{proj.title}</h3>
                      ...
                      <div className="flex gap-4 border-t border-slate-800/50 pt-4">
                        <a href={proj.githubUrl} target="_blank" rel="noreferrer" className="text-slate-500 hover:text-white transition-colors">
                          <Github size={20}/>
                        </a>
                        <a href={proj.deployUrl} target="_blank" rel="noreferrer" className="text-slate-500 hover:text-white transition-colors">
                          <Globe size={20}/>
                        </a>
                        <a href={proj.deployUrl} target="_blank" rel="noreferrer" className="ml-auto text-blue-500/50 group-hover:text-blue-400 transition-colors">
            ...
            <div className="col-span-12 lg:col-span-4 space-y-6">
              ...
              <div className="bg-gradient-to-br from-slate-900/80 to-slate-950 border border-slate-800 p-6 rounded-3xl">
                ...
                <p className="text-xs text-slate-500 mt-1">Tempo dedicado ao aprimoramento técnico nesta plataforma.</p>
              </div>
            </div>
          </div>
        </main>
